    ...
    const perksOptions = [
      ...
      const handleCheckboxChange = (ev) => {
        ...
        const updatedSelected = checked
                        ? [...selected,name] : selected.filter(item => item!==name);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {perksOptions.map(({ name, label, svgPath }) => (
          <label key={name} className="border rounded-2xl gap-2 p-4 flex items-center cursor-pointer">
            <input
              type="checkbox"
            ...
            <div className="flex items-center">
              <svg className="h-6 w-6 mr-2 text-gray-600" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                {svgPath}
              </svg>
